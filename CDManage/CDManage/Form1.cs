...
namespace CDManage
{
    ...
    {
        OleDbConnection CDconnection;
        public Form1()
        {
            InitializeComponent();

            CDconnection.ConnectionString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=D:\C#\CDProjects\CDManage";
        }
        ...
        {
            CDconnection.Open();
            OleDbCommand enterCD = new OleDbCommand();
            enterCD.Connection = CDconnection;
            enterCD.CommandText = "insert into CDdtb (Album,Artist,Genre,AlbumYear) values ('" + addAlbumBx.Text + "','" + addArtistBx + "','" 
                + addGenreBx + "','" + addYearBx + "')";

            enterCD.ExecuteNonQuery();
