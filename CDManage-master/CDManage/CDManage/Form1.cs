...
namespace CDManage
{
    ...
    public partial class Form1 : Form
    {
        ...
        string userPathStr = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Database/LoginDTB.accdb");
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection songConn = new OleDbConnection(songPathStr))
            {
                foreach (Cd x in CdList)
                {
                    string sqlQuery = "SELECT `Song Name` FROM SongListDTB where Album like @album and Artist like @artist";
        ...
        private void LoginBtn_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(userPathStr))
            {
                ...
                try
                {
                    string sqlQuery = "select `UserLevel` from LoginDTB where Username like @username and Password = @password";
                    OleDbCommand cmd = new OleDbCommand(sqlQuery, conn);
                    ...
                    while (reader.Read())
                    {
                        if (reader.HasRows)
                        {
                            ...
                            try
                            {
                                int.TryParse(usrLvlStr, out usrLvl);
                            }
                            catch
                            {
                                Debug.WriteLine("Error parsing user level");
                            } 
        ...
        private void signUpBtn_Click(object sender, EventArgs e)
        {
            using (OleDbConnection connLg = new OleDbConnection(userPathStr))
            {
                //MailAddress from = new MailAddress(txtNewUsrEmail.Text.Trim());
                try
                {
                    const int noEditPms = 1;
                    ...
                    while (reader.Read())
                    {
                        ...
                        /*try
                        {
                            var addr = new System.Net.Mail.MailAddress(txtNewUsrEmail.Text.Trim());
                        }
                        catch
                        {
                            newUsrLbl.Text = "Sorry, that email address is not valid.";
                        } */
                        else
                        {
        ...
        private void loginToolStripMenuItem_Click(object sender, EventArgs e)
        {
            //Switch to login Panel
            PanelSwitch(loginPnl.Name);
            loginPnl.Location = new Point(0, 10);
            loginPnl.Size = new Size(700, 550);
        }

        private void signUpToolStripMenuItem_Click(object sender, EventArgs e)
        {
            //Switch to Sign Up Panel
            PanelSwitch(SignUpPnl.Name);
            SignUpPnl.Location = new Point(0, 10);
            SignUpPnl.Size = new Size(700, 550);
        }
